...
export default {
  ...
  parameters: {
    docs: {
      description: {
        component: `InputText mendukung slot tambahan pada label melalui \`label-icon\`.

Slot ini dapat digunakan untuk menampilkan icon informasi, badge kecil, atau komponen tooltip di sebelah teks label.

\`\`\`vue
<InputText label="Nama Lengkap" v-model="value">
  <template #label-icon>
    <TooltipComponent
      class="ms-1"
      color="black"
      tooltipText="Masukkan nama sesuai identitas."
      placement="top"
      variant="dark"
    />
  </template>
...
\`\`\``,
  ...
  argTypes: {
    ...
    render: (args) => ({
          components: { InputText, TooltipComponent },
          setup() {
            const modelValue = ref(""); // Hubungkan dengan args
            const inputArgs = computed(() => {
              const rest = { ...args };
              delete rest.labelIcon;
              delete rest.labelTooltip;
              delete rest.prefix;
              delete rest.suffix;
              return rest;
            });
            return { args, inputArgs, infoIcon, modelValue };
          },
          template: `
            <InputText v-bind="inputArgs" v-model="modelValue">
              <template v-if="args.labelIcon || args.labelTooltip" #label-icon>
                <TooltipComponent
                  v-if="args.labelTooltip"
                  class="ms-1"
                  color="black"
                  :tooltipText="args.labelTooltip"
                  placement="top"
                  variant="dark"
                />
                <img
  ...
  export const WithLabelIcon = {
    ...
    parameters: {
      docs: {
        description: {
          story: "Menampilkan icon di sebelah label menggunakan slot `label-icon`.",
        },
      },
    },
  };
  ...
  export const WithLabelTooltip = {
    args: {
      label: "Nama Lengkap",
      placeholder: "Masukkan nama lengkap",
      labelTooltip: "Masukkan nama sesuai identitas yang berlaku.",
    },
    parameters: {
      docs: {
        description: {
          story: "Menggunakan slot `label-icon` untuk menampilkan `TooltipComponent` pada label.",
        },
      },
    },
  };
