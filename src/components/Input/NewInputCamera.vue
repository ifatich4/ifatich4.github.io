...
const handleSourceCameraClick = () => {
    ...
    previousFileSrc.value = fileSrc.value
    fileSrc.value = ''
...
const formatCapturedAt = (date = new Date()) => {
...
const handleCameraSnap = () => {
    ...
    if (isMobileView) {
        ...
        // 1. Ambil ukuran asli video dari perangkat
        const vW = video.value.videoWidth
        const vH = video.value.videoHeight

        // 2. Cari sisi terpendek agar bisa dibuat kotak sempurna (1:1)
        const minSize = Math.min(vW, vH)

        // 3. Hitung koordinat potong (X dan Y) agar posisinya pas di tengah-tengah
        const startX = (vW - minSize) / 2
        const startY = (vH - minSize) / 2

        // 4. Tetapkan ukuran Canvas (Misal standar 532x532 piksel, rasio 1:1)
        // Perhatian: Wajib menentukan width dan height canvas, kalau tidak akan default ke 300x150
        const CANVAS_SIZE = 532
        canvas.width = CANVAS_SIZE
        canvas.height = CANVAS_SIZE

        // 5. Eksekusi pemotongan (cropping)
        // Penjelasan parameter: (sumber, potong_x, potong_y, ukuran_potong_w, ukuran_potong_h, letak_x, letak_y, ukuran_canvas_w, ukuran_canvas_h)
        ctx.drawImage(
            video.value,
            startX,
            startY,
    ...
    if (props.useWatermark) {
        capturedAt.value = formatCapturedAt()
        const watermarkHeight = 40

        // Background Watermark
        ctx.fillStyle = 'rgba(37, 37, 40, 0.74)'
        ...
        // Posisikan text persis di tengah background watermark
        const textYPos = canvas.height - (watermarkHeight / 2)
        const paddingX = 16 // Jarak dari pinggir kiri dan kanan

        const leftText = `Diambil oleh : ${props.userName}`
        const rightText = `Waktu Ambil Foto : ${capturedAt.value}`

        // Gambar Teks Kiri
        ctx.textAlign = 'left'
        ctx.fillText(leftText, paddingX, textYPos)

        // Gambar Teks Kanan
        ctx.textAlign = 'right'
        ctx.fillText(rightText, canvas.width - paddingX, textYPos)
    }

    // Melanjutkan sisa logic Anda sebelumnya
    locationData.value = props.deviceCoordinate
        ? {
...
const handleCameraDialogValueChange = (isShowing) => {
    if (!isShowing) {
        ...
        facingMode.value = 'environment'
        snappedCameraPict.value = ''
        if (!fileSrc.value && previousFileSrc.value) {
            fileSrc.value = previousFileSrc.value
            previousFileSrc.value = ''
        }
    }
}
...
const handleRetakeGalleryClick = async () => {
    fileSrc.value = ''
    previewDialog.value = false
    await nextTick()
    fileInput.value.click()
                ...
                <div>
                    <div class="custom-file-upload__box-input-icon">
                        ...
                        <div v-else class="d-flex gap-2">
                            <img src="../../assets/icon/upload.svg" alt="" draggable="false" />
            ...
            <div>
                ...
                <div class="footer-button">
                    <Button
                        ...
                        type="neutral"
                        :label="isFromCamera ? 'Ambil Ulang Foto' : 'Pilih Ulang Foto'"
                        :id="`${$attrs.id}_cameraRetake`"
                        class="w-100"
                    />
                    <Button
                        v-if="showPreviewConfirmButton"
                        @click="handleConfirmPreview"
                        type="primary"
                        label="Gunakan Foto"
                        :id="`${$attrs.id}_previewConfirm`"
                        class="w-100"
                    />
                    <Button
