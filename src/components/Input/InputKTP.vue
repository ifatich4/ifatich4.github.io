<script setup>
    ...
    const props = defineProps({
        ...
        /**
         * Add "Diambil Oleh: " prefix to timestamp overlay userName
         */
        takenPrefix: {
            type: Boolean,
            default: false
    ...
    // Computed: Display format untuk userName (opsional prefix "Diambil Oleh:")
    const displayUserName = computed(() => {
        const name = props.userName || 'unknown'
        if (props.takenPrefix) {
            return name.startsWith('Diambil Oleh:') ? name : `Diambil Oleh: ${name}`
        }
        return name
    })
    ...
    <div v-if="fileSrc && previewDialog">
        ...
        <BModal v-if="!isMobile" v-model="previewDialog" :title="props.mode === 'preview' ? 'Lihat' : 'Lihat Foto'" size="lg"
            ...
            <div>
                ...
                <div class="footer-button" v-if="props.mode !== 'preview' || $slots.footer">
                    <slot name="footer" :close="() => previewDialog = false">
